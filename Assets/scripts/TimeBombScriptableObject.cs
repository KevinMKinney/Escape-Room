 ...
 * This scriptable object acts as a data container for the Bomb script. It manages bomb events and slows down time when a bomb succesfully explodes.
 */
//This line alows an instance to be created in the unity editor by right clicking-> Create -> ScriptableObjects -> Time Bomb Scriptable Object
...
{
    //This curve represnts how slow time is over time. All values are expected to be 0 (Not time passing) to 1 (Time passing normally)
    ...
    public override void AddEventHandlers()
    {
        ...
        explodeEvent.AddResultListener((timeBomb, cancelableObject) =>
        {
            ...
            {
                //Get a instance of a MonoBehaviour to start the coroutine SlowDownTime. ScriptableObject do not StartCoroutine defined in the class hierarchy.
                TargetScriptManager.instance.StartCoroutine(SlowDownTime(timeCurve));
            }
        });
    }

    //Slows down time using the curve to detimine when time slows down and by how much.
    public IEnumerator SlowDownTime(AnimationCurve curve)
    {
        ...
        //Save all of the values on the player for movment. When slowing time down we need to keep these variables updated so the player keeps moving normally.
        ...
        //Keep updating time untill we reach the end of the curve.
        while (time < endTime)
        {
            ...
            //Gravity is special, and needs to be exponentally increased and decreased to keep the player from flying into the sky.
    ...
    public override CancelableObject InvokeExplodeEvent(Bomb bomb)
    {
        ...
        //If time is alreay being slowed down then the event needs to be canceled
        if (isTimeSlow)
